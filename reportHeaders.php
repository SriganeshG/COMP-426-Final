...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
       ...
       ($path_components[1] != "")){
        if(isset($_REQUEST['type'])){
            ...
            foreach($report_array as $reportID){
                $report_array_JSON[$reportID] = report::findById($reportID)->getJSONHeader();
            }
            ...
            header("Content-type: application/json");
            print(report::findById($reportID)->getJSONHeader());
            exit();
        } else {
            //Do it by building
    ...
    foreach($reportIds as $id){
        $reportArray[$id] = report::findById($id)->getJSONHeader();
    }
    header("Content-type: application/json");
